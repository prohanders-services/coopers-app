...
export default function CreateOffer({ goBack, navigate }) {
  const { addAdminOffer } = useApp()
  ...
  const canPublish = title.trim() && value.trim()

  const publish = () => {
    if (!canPublish) return
    addAdminOffer({ title, reductionType, value, maxTotal, endDate, category })
    setSaved(true)
    setTimeout(() => { setSaved(false); goBack() }, 2000)
  }
    ...
    <div style={{ width: '100%', height: '100%', background: ABKG, position: 'relative', overflow: 'hidden', color: C.ink, display: 'flex', flexDirection: 'column' }}>
      ...
      {saved && (
        <div style={{
          ...
          <div style={{ marginTop: 16, fontSize: 20, fontWeight: 800 }}>Offre publiée !</div>
          <div style={{ marginTop: 8, fontSize: 14, opacity: 0.8 }}>Visible par les clients Coopers</div>
        </div>
      )}

      <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '0 16px 40px', paddingBottom: 120 }}>
        {/* Header */}
        <div style={{ paddingBottom: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: -0.3 }}>Créer une offre</div>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                {isLoyalty && <span style={{
                  padding: '2px 8px', borderRadius: 999, fontSize: 10, fontWeight: 700,
                  background: `${AB}15`, color: AB,
                }}>🔒 Fidélité</span>}
              </div>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button
            ...
            style={{
              width: '100%', height: 120, borderRadius: 16, border: `2px dashed ${hasPhoto ? AB : C.gray200}`,
              background: hasPhoto ? `${AB}08` : '#fff', cursor: 'pointer',
              display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
              gap: 8, fontFamily: 'inherit',
            }}
          >
            <span style={{ fontSize: 30 }}>{hasPhoto ? '🖼️' : '📷'}</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: hasPhoto ? AB : C.gray400 }}>
              {hasPhoto ? 'Photo sélectionnée — appuyez pour changer' : 'Ajouter une photo de l\'offre'}
            </span>
          </button>
        ...
        <div style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button
            ...
            style={{
              ...
              background: canPublish ? AB : C.gray200,
              color: canPublish ? '#fff' : C.gray400,
              border: 'none', cursor: canPublish ? 'pointer' : 'not-allowed',
              fontSize: 15, fontWeight: 800, fontFamily: 'inherit',
              boxShadow: canPublish ? `0 6px 20px rgba(37,99,235,0.35)` : 'none',
            }}
          >
            ✅ Publier l'offre
          </button>
          <button
            ...
            style={{
              ...
              background: '#fff', color: AB,
              border: `1.5px solid ${AB}40`, cursor: 'pointer',
